...
class ArrowLaunch(private val player: Player, private val artilleryEntity: Entity, private val plugin: JavaPlugin) : Skill(player, plugin) {
    override fun start() {
    ...
    private fun run() {
        val task = object : BukkitRunnable() {
            override fun run() {
                ...
                currentLocation.playSound(Sound.ENTITY_ARROW_SHOOT, 1.0f, 1.3f)
                currentLocation.spawn<Arrow> {
                    it.velocity = vector.multiply(2)
                }
            }
